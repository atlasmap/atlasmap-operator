...
func atlasMapConsoleLinkTest(t *testing.T, f *framework.Framework, ctx *framework.TestCtx) error {
	namespace, err := ctx.GetNamespace()
	if err != nil {
		return fmt.Errorf("could not get namespace: %v", err)
	}

	crName := "test-atlasmap-consolelink"
	...
	consoleLinkText := "AtlasMap - Test Atlasmap Consolelink"
	consoleLinkHref := "https://" + atlasMapRoute.Spec.Host

	consoleLink := &consolev1.ConsoleLink{}
	if err := f.Client.Get(goctx.TODO(), types.NamespacedName{Name: util.ConsoleLinkName(exampleAtlasMap), Namespace: namespace}, consoleLink); err != nil {
		return err
	}

	if consoleLink.Spec.Link.Text != consoleLinkText {
		return fmt.Errorf("expected ConsoleLink text to match %s but got %s", consoleLinkText, consoleLink.Spec.Link.Text)
	}

	if consoleLink.Spec.Link.Href != consoleLinkHref {
		return fmt.Errorf("expected ConsoleLink href to match %s but got %s", consoleLinkHref, consoleLink.Spec.Link.Href)
	}

	// Verify that the ConsoleLink is deleted with the AtlasMap CR
	if err := f.Client.Delete(goctx.TODO(), exampleAtlasMap); err != nil {
		return err
	}

	if err := e2eutil.WaitForDeletion(t, f.Client.Client, consoleLink, retryInterval, timeout); err != nil {
		return err
	}
...
func AtlasMapCluster(t *testing.T) {
	...
	isOpenShift, _ := util.IsOpenShift(f.KubeConfig)
	if isOpenShift && util.IsOpenShift43Plus(f.KubeConfig) {
		tests = append(tests, atlasMapConsoleLinkTest)
	}
